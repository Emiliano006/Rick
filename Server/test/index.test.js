...
describe("Test de RUTAS", ()=>{
    describe("GET /rickandmorty/character/:id", ()=>{
        ...
        it("Si hay un error responde con status: 500", async ()=>{
            await agent.get('/rickandmorty/character/45654').expect(500);
        })
    });
    describe("GET /rickandmorty/login", ()=>{
        it("La información de login es correcta", async ()=>{
            const response = (await agent.get("/rickandmorty/login?email=ejemplo@gmail.com&password=unaPassword")).body
            expect(response.acces).toEqual(true)
        });
        it("La información de login es incorrecta", async ()=>{
            const response = (await agent.get("/rickandmorty/login?email=pepito@gmail.com&password=123")).body
            expect(response.acces).toEqual(false)
        });
    });
...
describe("DELETE /rickandmorty/fav/:id", ()=>{
    ...
    it("Devuelve el arreglo correspondiente si no se elimina ningun personaje", async ()=>{
        const response = (await agent.delete("/rickandmorty/fav/8454")).body;
        expect(response).toContainEqual(character1);
        expect(response).toContainEqual(character2);
    });
